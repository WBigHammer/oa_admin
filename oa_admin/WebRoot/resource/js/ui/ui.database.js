	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				var $addBut;
				if($table.attr('locationButton')=="insertAfter"){
					$addBut = $('<button type="button" class="btn btn-primary addItem" style="margin:5px;">'+addButTxt+'</button>').insertAfter($table);
				}else{
					$addBut = $('<button type="button" class="btn btn-primary addItem" style="margin:5px;">'+addButTxt+'</button>').insertBefore($table);
				}
				$addBut.click(function(){
					$.addItemDetail($tbody,trHtml);
					if($table.attr('clickEvent')){
						var $clickEvent=eval('(' + $table.attr('clickEvent') + ')');
						if(!$clickEvent($table))return false;
					}
