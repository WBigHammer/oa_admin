...
function validateSubmitForm(form){
	...
	$("input[fromDatagrid]",$form).each(function(){
		...
		if($dg.length==1){
			...
			var rows=new Array()
			var iiii_=0;
			$dg.datagrid('getPanel').find(".datagrid-view .datagrid-body tr input[name='ck']").each(function(index,element){
				
					if(this.checked == true){
						rows[iiii_] = $dg.datagrid('getRows')[index];
						iiii_++;
					}
			});
			...
			$.ajax({
				...
				success:function(json){
					if(json!=null){
						...
						submitSuccessCallback($form,json);
			...
			$this.click(function(event){
				...
				if(url&&url.length>0){
					...
					if(gid){
						...
						var check=0;
						var checkedId;
						$dg.datagrid('getPanel').find(".datagrid-view .datagrid-body tr input[name='mergechecked']").each(function(index,element){
...
//							console.log(pm);
							...
							var targetType=$this.attr("targetType");
							if (title) {
								Msg.confirm('提示',"<span style='color:red'>[&nbsp;"+l+"&nbsp;]</span>行数据将会被合并。<br/>"+title, function(r){
						               if (r){
						            	   if(targetType=="export"){
						            		   selectedTodo_export(url,pm)
						            	   }else if(targetType=="print"){
								...
								Msg.prompt('请输入信息', "<span style='color:red'>[&nbsp;"+l+"&nbsp;]</span>行数据将会被合并。<br/>"+titleText, function(r){
					                if (r){
					                   pm["text"]=r;
					                   selectedTodo_doPost(url,pm,$dg,callback);
					                }
 ...
 * 返回 为ajaxJsonmessage
 */
function ajaxSubmitmessage(url,data,message,fn){
